...
def processar_dados_alertas():
    ...
    for _, row in df_hist.iterrows():
        ...
        # 🛠️ CAPTURA DE PENDÊNCIA ULTRA-ROBUSTA:
        # Analisa a linha inteira para ver se o termo "PENDENTE" existe em algum lugar
        linha_completa_txt = (raw_timestamp + " " + raw_data_entrega + " " + qtd_val).upper()
        
        if "PENDENTE" in linha_completa_txt:
            status_assinatura = "Pendente"
            # Extrai apenas os primeiros 10 caracteres (ex: "2026-07-06"), eliminando o " - PENDENTE"
            if len(raw_data_entrega) >= 10:
                raw_data_entrega = raw_data_entrega[:10].strip()
        else:
            status_assinatura = "Assinado"
            
        # Converte a data de entrega forçando apenas a parte da data (sem horas residuais)
        ...
        # Zera horas, minutos e segundos para evitar problemas de filtragem na tela
        dt_entrega_parsed = dt_entrega_parsed.normalize()
        ...
        # Compatibilidade de Departamento estruturada
        depto = "Não Informado"
...
if menu == "Dashboard":
    ...
    with col_d3:
        if not df_base_completa.empty:
            ...
            st.download_button(
                ...
                mime="text/csv",
                type="secondary",
                help="Clique para baixar os registros deste período."
            )
    ...
    if df_base_completa.empty:
        ...
        if df_dash.empty:
            st.warning("⚠️ Não existem lançamentos no intervalo selecionado.")
        else:
            ...
            with col_g1:
                ...
                fig_barras = px.bar(df_depto, x='Departamento', y='Qtd', text='Qtd', labels={'Qtd': 'Qtd', 'Departamento': 'Setor'}, color_discrete_sequence=['#2c3e50'])
                fig_barras.update_traces(textposition='outside')
                fig_barras.update_layout(margin=dict(l=20, r=20, t=10, b=20), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
                st.plotly_chart(fig_barras, use_container_width=True)

            with col_g2:
                st.markdown("##### Concentração de Pendências por Setor")
                ...
                if df_pend_depto.empty:
                    st.success("🎉 Zero pendências acumuladas no período!")
                else:
                    fig_pizza = px.pie(df_pend_depto, values='Pendências', names='Departamento', hole=0.4, color_discrete_sequence=px.colors.qualitative.Pastel)
                    fig_pizza.update_traces(textinfo='percent+label', pull=[0.05] * len(df_pend_depto))
                    fig_pizza.update_layout(margin=dict(l=20, r=20, t=10, b=20), showlegend=False)
